alarm[0] = ChunkCollectTime;
x = ObCharacter.x;
y = ObCharacter.y;

var _pad = 5;
var _bbox_left, _bbox_top, _bbox_right, _bbox_bottom, _cull;
with(ObCulling){
	...
	_cull = point_distance(x, y, other.x, other.y) >= other.ChunkSize;
...
repeat(ds_list_size(CullList)){
	_inst = CullList[| i];
	_cull = !(point_distance(_inst[1], _inst[2], x, y) <= ChunkSize);
